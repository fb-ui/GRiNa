...
void StringToMap(const std::string filename, std::vector<GR_RESUNIT> *v)
{
    std::ifstream file;
    file.open(filename);
    while (file.eof())
...
void GR_ResourceManager::Load(const std::string &filename)
{
...
{
    this->res_list = new std::vector<GR_RESUNIT>();
    this->Load(filename);
}
...
SDL_RWops* GR_ResourceManager::GetResource(const std::string &type, const std::string &name)
{
    for(this->iter=this->res_list->begin();iter!=this->res_list->end();iter++)
    {
